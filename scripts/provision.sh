...
Options:
  ...
  --fallback-api-url <url>     Fallback LLM API URL (default: from sdkconfig)
  --fallback-model <model>     Fallback model name (default: from sdkconfig)
  --fallback-backend <backend> Fallback LLM backend (anthropic | openai | openrouter | ollama)
  --fallback-api-key <key>     Fallback LLM API key
...
if [ -z "$FALLBACK_API_URL" ] && [ "$ASSUME_YES" != true ]; then
    read -r -p "Fallback LLM API URL (default: ${CONFIG_ZCLAW_LLM_FALLBACK_URL:-http://localhost:11434/v1}): " FALLBACK_API_URL_INPUT
    FALLBACK_API_URL="${FALLBACK_API_URL_INPUT:-${CONFIG_ZCLAW_LLM_FALLBACK_URL:-http://localhost:11434/v1}}"
else
    FALLBACK_API_URL="${FALLBACK_API_URL:-${CONFIG_ZCLAW_LLM_FALLBACK_URL:-http://localhost:11434/v1}}"
fi
...
if [ -z "$FALLBACK_BACKEND" ] && [ "$ASSUME_YES" != true ]; then
    read -r -p "Fallback LLM backend (default: ${CONFIG_ZCLAW_LLM_FALLBACK_BACKEND:-}): " FALLBACK_BACKEND_INPUT
    FALLBACK_BACKEND="${FALLBACK_BACKEND_INPUT:-${CONFIG_ZCLAW_LLM_FALLBACK_BACKEND:-}}"
else
    FALLBACK_BACKEND="${FALLBACK_BACKEND:-${CONFIG_ZCLAW_LLM_FALLBACK_BACKEND:-}}"
fi

if [ -z "$FALLBACK_API_KEY" ] && [ "$ASSUME_YES" != true ]; then
    read -r -p "Fallback LLM API key (default: ${CONFIG_ZCLAW_LLM_FALLBACK_API_KEY:-}): " FALLBACK_API_KEY_INPUT
    FALLBACK_API_KEY="${FALLBACK_API_KEY_INPUT:-${CONFIG_ZCLAW_LLM_FALLBACK_API_KEY:-}}"
else
    FALLBACK_API_KEY="${FALLBACK_API_KEY:-${CONFIG_ZCLAW_LLM_FALLBACK_API_KEY:-}}"
fi
...

{
    ...
    if [ -n "$FALLBACK_BACKEND" ]; then
        printf "llm_fallback_backend,data,string,%s\\n" "$(csv_escape "$FALLBACK_BACKEND")"
    fi
...
echo "  Model:     $MODEL"
    ...
    echo "  Fallback API URL: $FALLBACK_API_URL"
    echo "  Fallback Model: $FALLBACK_MODEL"
    if [ -n "$FALLBACK_BACKEND" ]; then
        echo "  Fallback Backend: $FALLBACK_BACKEND"
    fi
    if [ -n "$FALLBACK_API_KEY" ]; then
        echo "  Fallback API Key: <set>"
    fi
if [ -n "$API_URL" ]; then
    echo "  API URL:   $API_URL"
fi
